...
describe("Button block tab tests", () => {
  ...
  it("Create block with type button", async () => {
    ...
    await driver.wait(until.urlIs(`${Config.serverUrl}/#/Block`), 2800);
  ...
  it("Element button already exist", async () => {
    ...
    await driver.sleep(60);
    await driver
      .wait(until.elementLocated(By.css("a[aria-label^='Створити']")), 2800)
      .click();
  ...
  it("Input invalid value for fields", async () => {
    ...
    expect(fontweightError).to.eql("Мінімальне значення 1");
