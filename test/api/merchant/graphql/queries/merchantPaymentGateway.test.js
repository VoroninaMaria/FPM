  ...
  describe("authentication", () => {
      ...
      await Database("merchant_payment_gateways")
        .insert([
          ...
          },
          {
            ...
            payment_gateway_id: pumbGateway.id,
            config: {},
            status: MERCHANT_PAYMENT_GATEWAY_STATUSES.disabled.name,
          },
        ])
        .onConflict(["merchant_id", "name"])
        .merge();
    ...
    context("All Merchant Payment Gateways", () => {
      ...
      it("Get allMerchantPaymentGateways with valid filter(id) and without pagination", async () => {
        const mpg = await Database("merchant_payment_gateways")
          .where({ merchant_id: bolt.id, payment_gateway_id: pumbGateway.id })
          .first();
      ...
      it("Get allMerchantPaymentGateways with valid filter(ids) and without pagination", async () => {
        const mpgs = await Database("merchant_payment_gateways").where({
          merchant_id: bolt.id,
        });
      ...
      it("Get allMerchantPaymentGateways with other merchant_id filter id", async () => {
        const mpg = await Database("merchant_payment_gateways")
          .where({
            merchant_id: uklon.id,
            payment_gateway_id: pumbGateway.id,
      ...
      it("Get allMerchantPaymentGateways with other merchant_id filter ids", async () => {
        const mpgs = await Database("merchant_payment_gateways").where({
          merchant_id: uklon.id,
        });
    ...
    context("All Merchant Payment Gateways Meta", () => {
      ...
      it("Get allMerchantPaymentGatewaysMeta with other merchant_id filter id", async () => {
        const mpg = await Database("merchant_payment_gateways")
      ...
      it("Get allMerchantPaymentGatewaysMeta with other merchant_id filter ids", async () => {
        const mpgs = await Database("merchant_payment_gateways").where({
          merchant_id: uklon.id,
        });
