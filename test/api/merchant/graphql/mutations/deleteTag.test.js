...
import { DELETE_TAG_MUTATION as DELETE_TAG } from "@local/test/api/mutations.js";
...
describe("Merchant GraphQL", () => {
  ...
  describe("mutation { deleteTag }", () => {
    it("Should delete tag with valid id provided", async () => {
      const { id } = await Database("tags").first();

      variables.id = id;

      await accountGraphQLRequest(requestBody(DELETE_TAG), (res) => {
        const {
          body: {
            data: { deleteTag },
          },
        ...
        expect(res.body.data).to.have.property("deleteTag");
        expect(deleteTag).to.have.property("id", id);
        expect(deleteTag).to.have.property("name", "nice guy");
      });

      return accountGraphQLRequest(requestBody(DELETE_TAG), (res) => {
        expect(res).to.have.status(200);
        expect(res.body).to.have.property("errors");
        expect(res.body.errors[0]).to.have.property("message");
        expect(res.body.errors[0].message).to.include("tag_not_found");
      });
    });

    it("Should return error when no id provided", () =>
      accountGraphQLRequest(requestBody(DELETE_TAG), (res) => {
