...
import { MERCHANT_UPDATE_GAS_BRAND_MERCHANT_MUTATION as UPDATE_GAS_BRAND_MERCHANT } from "@local/test/api/mutations.js";
import {
  GAS_BRAND_STATUSES,
  MERCHANT_STATUSES,
...
describe("Merchant GraphQL", () => {
  ...
  describe("mutation { updateGasBrandMerchant }", () => {
    ...
    it("Should not update status with valid id, merchant_id, gas_brand_id, fuels and status provided (but current status blocked)", async () => {
    ...
    it("Should return error when no fuels provided", async () => {
      variables.id = gbm.id;
      variables.gas_brand_id = gbm.gas_brand_id;
      variables.status = gbm.status;
