...
describe("Admin GraphQL", () => {
  describe("authentication", () => {
    ...
    context("All Gas Brands Meta", () => {
      ...
      it("Get allGasBrandMerchantsMeta with sortField which does not exist", () => {
        ...
        return accountGraphQLRequest(
          ...
          (res) => {
            ...
            expect(_allGasBrandMerchantsMeta.count).to.eq(1);
          }
        );
      });
      ...
      it("Get allGasBrandMerchantsMeta with valid pagination and filter gas_brand_id", async () => {
        const { gas_brand_id } = await Database("gas_brand_merchants")
          .where({ merchant_id: merchant1.id })
          .first();
