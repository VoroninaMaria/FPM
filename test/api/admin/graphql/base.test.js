...
describe("Admin GraphQL", () => {
  describe("authentication", () => {
    beforeEach(async () =>
      Database("admins").insert({
        login: "offtop",
        encrypted_password: await encryptPassword("123123"),
      })
    );

    afterEach(() => Database("admins").del());

    it("is expected to return status 403 if unauthenticated", () =>
      accountGraphQLRequest(requestBody(ADMIN_SELF_QUERY), "", (res) => {
        ...
        ({ body: { token } }) =>
          accountGraphQLRequest(
            ...
            (res) => {
              ...
              expect(self).to.have.property("login", self.login);
    ...
    describe("Analyze INTROSPECTION_QUERY", () => {
      ...
      it("is expected to have proper fields for Admin", () => {
        // Check AdminType
        const adminType = res.body.data.__schema.types.find(
          (element) => element.name === "Admin"
        );
        const adminTypeFieldNames = adminType.fields
          .map(({ name }) => name)
          .sort();

        expect(adminTypeFieldNames).to.eql([
          "created_at",
          "id",
          "login",
          "updated_at",
