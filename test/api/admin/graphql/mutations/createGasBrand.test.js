...
import { ADMIN_CREATE_GAS_BRAND_MUTATION as CREATE_GAS_BRAND } from "@local/test/api/mutations.js";
import {
  GAS_BRAND_STATUSES,
  MERCHANT_STATUSES,
...
describe("Admin GraphQL", () => {
  ...
  describe("mutation { createGasBrand }", () => {
    it("Should create brand with valid name, status and logo_file_id", async () => {
      const file = await Database("files").first().returning("*");

      variables.name = "Minions";
      variables.logo_file_id = file.id;
      variables.status = GAS_BRAND_STATUSES.active.name;

      return accountGraphQLRequest(requestBody(CREATE_GAS_BRAND), (res) => {
        const {
          body: {
            data: { createGasBrand },
          },
        ...
        expect(res.body.data).to.have.property("createGasBrand");
        expect(createGasBrand).to.have.property("name", variables.name);
      });
    });
    ...
    it("Should return error when logo_file_id has wrong type", async () => {
      variables.name = "Businki";
