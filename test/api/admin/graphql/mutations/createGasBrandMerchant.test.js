...
import { ADMIN_CREATE_GAS_BRAND_MERCHANT_MUTATION as CREATE_GAS_BRAND_MERCHANT } from "@local/test/api/mutations.js";
import {
  GAS_BRAND_STATUSES,
  MERCHANT_STATUSES,
...
describe("Admin GraphQL", () => {
  ...
  describe("mutation { createGasBrand }", () => {
    it("Should create gas_brand_merchant with valid merchant_id, gas_brand_id, status", async () => {
      const merchant = await Database("merchants").first().returning("*");
      const gas_brand = await Database("gas_brands")
        .where({ name: "Kotiki1" })
        .first()
        .returning("*");
    ...
    it("Should return error when merchant_id is null", async () => {
      ...
      return accountGraphQLRequest(
        ...
        (res) => {
          ...
          expect(res.body.errors[0]).to.have.property("message");
          expect(res.body.errors[0].message).to.include(
            'Variable "$merchant_id" of non-null type "ID!" must not be null'
          );
        }
      );
    });
