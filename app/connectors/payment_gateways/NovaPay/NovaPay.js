...
class NovaPay {
    ...
    async createSession(merchant_id, client_phone) {
        return this.skeletonPost("createSession", JSON.stringify({
            merchant_id: merchant_id,
            client_phone: client_phone,
        }));
    }
    async addPayment(merchant_id, session_id, amount) {
        return this.skeletonPost("addPayment", JSON.stringify({
            merchant_id: merchant_id,
    ...
    async voidSession(merchant_id, session_id) {
        return this.skeletonPost("voidSession", JSON.stringify({
            merchant_id: merchant_id,
            session_id: session_id,
        }));
    }
    async completeHold(merchant_id, session_id) {
        return this.skeletonPost("completeHold", JSON.stringify({
            merchant_id: merchant_id,
            session_id: session_id,
        }));
    }
    ...
    async expireSession(merchant_id, session_id) {
        return this.skeletonPost("expireSession", JSON.stringify({
            merchant_id: merchant_id,
            session_id: session_id,
        }));
    }
    async confirmDeliveryHold(merchant_id, session_id) {
        return this.skeletonPost("confirmDeliveryHold", JSON.stringify({
            merchant_id: merchant_id,
            session_id: session_id,
        }));
    }
    async printExpressWaybill(merchant_id, session_id) {
        return this.skeletonPost("printExpressWaybill", JSON.stringify({
            merchant_id: merchant_id,
            session_id: session_id,
        }));
    }
    async getStatus(merchant_id, session_id) {
        return this.skeletonPost("getStatus", JSON.stringify({
            merchant_id: merchant_id,
            session_id: session_id,
        }));
    }
    async deliveryInfo(merchant_id) {
        return this.skeletonPost("deliveryInfo", JSON.stringify({
            merchant_id: merchant_id,
        }));
    }
    async deliveryPrice(merchant_id, recipient_city, recipient_warehouse, volume_weight, weight, amount) {
        return this.skeletonPost("deliveryPrice", JSON.stringify({
            merchant_id: merchant_id,
